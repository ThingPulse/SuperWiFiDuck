...
home_dir=$(echo ~)
current_dir=$(pwd)
build_dir="$current_dir/.pio/build/thingpulse-pendrive-s3"

bootloader="$build_dir/bootloader.bin"
partitions="$build_dir/partitions.bin"
boot_app="$home_dir/.platformio/packages/framework-arduinoespressif32/tools/partitions/boot_app0.bin"
esptool="$home_dir/.platformio/packages/tool-esptoolpy/esptool.py"
firmware="$build_dir/firmware.bin"
app="$build_dir/app-firmware.bin"
...
python3 $esptool --chip ESP32-S3 merge_bin -o "$app" --flash_mode dio --flash_size 4MB 0x0000 "$bootloader" 0x8000 "$partitions" 0x10000 "$firmware"
